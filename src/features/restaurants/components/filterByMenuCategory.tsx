...
/*
    if restaurant selected then filter the menu based on it else all
*/
  const filters: Record<string, string> | undefined =
    filter?.name === "restaurant" && filter?.value
      ? { restaurant: filter.value.toString() }
      : undefined;

  const { data, isLoading, isFetching } = useMenuCategories({
    searchQuery: debouncedSearchValue,
    filters,
  });
  const menuFoodCategoryOptions =
    isLoading || isFetching
      ? []
      : data?.map((f) => ({ name: f.name, value: f.name })) || [];
